...
def merge_lrtlist(lenlist, rtlist):
    ...
    lrtlist = torch.cat([lenlist, rtlist], dim=2)
    return lrtlist
...
def get_xyzlist_from_lenlist(lenlist):
    B, N, D = list(lenlist.shape)
    assert(D==3)
    lx, ly, lz = torch.unbind(lenlist, dim=2)

    xs = torch.stack([lx/2., lx/2., -lx/2., -lx/2., lx/2., lx/2., -lx/2., -lx/2.], dim=2)
    ys = torch.stack([ly/2., ly/2., ly/2., ly/2., -ly/2., -ly/2., -ly/2., -ly/2.], dim=2)
    zs = torch.stack([lz/2., -lz/2., -lz/2., lz/2., lz/2., -lz/2., -lz/2., lz/2.], dim=2)

    # these are B x N x 8
    xyzlist = torch.stack([xs, ys, zs], dim=3)
