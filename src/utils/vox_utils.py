...
class Vox_util(object):
    ...
    def get_mem_T_ref(self, B, Z, Y, X, assert_cube=False, device='cuda'):
        ...
        mem_T_ref = basic_utils.matmul2(mem_T_center, center_T_ref)
    ...
    def warp_tiled_to_mem(self, rgb_tileB, pixB_T_camA, camB_T_camA, Z, Y, X, DMIN, DMAX, assert_cube=False):
        ...
        xyz_pixB = torch.stack([x_pixB, y_pixB, z_tileB], dim=2)
        xyz_pixB = torch.reshape(xyz_pixB, [B, Z, Y, X, 3])
    ...
    def xyz2circles(self, xyz:torch.Tensor, radius, Z, Y, X, soft=True, already_mem=True, also_offset=False, grid=None):
        # xyz is B x N x 3
        ...
        if grid is None:
            grid_z, grid_y, grid_x = basic_utils.meshgrid3d(B, Z, Y, X, stack=False, norm=False, device=xyz.device.type)
    ...
    def xyz2circles_bev(self, xyz, radius, Z, Y, X, already_mem=True, also_offset=False):
        ...
        grid_z, grid_x = basic_utils.meshgrid2d(B, Z, X, stack=False, norm=False, device=xyz.device.type)
        # note the default stack is on -1
